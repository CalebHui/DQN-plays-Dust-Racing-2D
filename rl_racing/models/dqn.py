...
class DqnModel(AbstractModel):
    def __init__(self, env, network_type='CnnDQN', epsilon_start = 1, epsilon_final = 0.01, epsilon_decay = 400000, gamma = 0.99, batch_size = 64, buffer_size = 20000, tau=4):
        super().__init__(env)
    ...
    def reset(self):
        ...
        if self.network_type == 'CnnLstmDQN':
            self.current_model = CnnLstmDQN((self.tau, *self.env.input_shape), len(self.env.action_space))
            self.target_model = CnnLstmDQN((self.tau, *self.env.input_shape), len(self.env.action_space))
    ...
    def train(self, episodes = 1000000):
        ...
        for episode in tr:
            ...
            rewards = []
            for i in range(self.frame_skipping-1):
            ...
            if self.training_episodes % 2000 == 0:
                self.update_target()
    ...
    def play(self, episodes = 1):
        ...
        while episode < episodes:
            ...
            action = self.current_model.act(state, epsilon, self.env.action_space)
            rewards = []
            for i in range(frame_skipping-1):
    ...
    def compute_td_loss(self, batch_size):
        ...
        #print('next_q_state_values:{}'.format(next_q_state_values))
